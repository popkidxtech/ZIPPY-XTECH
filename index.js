...
const port = process.env.PORT || 9090;
  ...
  let up = `*Hi Owner 👋😄, Congrats Popkid Xtech  Connected Successfully!* 
  ...
  conn.sendMessage(conn.user.id, { image: { url: `https://files.catbox.moe/nw3mmc.jpg` }, caption: up })
  }
  })
 conn.ev.on('creds.update', saveCreds)
  ...
  const udp = botNumber.split('@')[0];
    ...
    if (isCreator && mek.text.startsWith('%')) {
					...
					if (!code) {
						reply(
							`Provide me with a query to run Lord!`,
						);
						return;
					}
					try {
